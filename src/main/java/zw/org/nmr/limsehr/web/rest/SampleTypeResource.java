...
import zw.org.nmr.limsehr.service.LimsSampleTypeService;
import zw.org.nmr.limsehr.service.LimsSampleTypeService;
 ...
 * REST controller for managing sample-types.
 * <p>
 * This class accesses the {@link SampleType} entity, and needs to fetch its collection of authorities.
 * <p>
...
public class SampleTypeResource {
    ...
    /**
     * {@code POST  /sample-types}  : Creates a new test.
     * <p>
     *
    ...
    public ResponseEntity<SampleType> createSampleType(@Valid @RequestBody SampleType labTest) throws URISyntaxException {
        ...
        } else if (limsSampleTypeService.findById(labTest.getId()).isPresent()) {
            throw new RecordAlreadyExistException();
        } else {
            SampleType lTest = limsSampleTypeService.save(labTest);
    ...
    /**
     * {@code PUT /sample-types} : Updates an existing Test.
     *
    ...
    public ResponseEntity<SampleType> updateSampleTypes(@Valid @RequestBody SampleType sampleType) {
        log.debug("REST request to update Laboratory : {}", sampleType);
        SampleType updatedTest = limsSampleTypeService.save(sampleType);
    ...
    /**
     * {@code GET /sample-types} : get all tests.
     *
     * @param pageable the pagination information.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body all tests.
     */
    @GetMapping("/sample-types")
    public ResponseEntity<List<SampleType>> getAllSampleTypes(@org.springdoc.core.annotations.ParameterObject Pageable pageable) {
        final Page<SampleType> page = limsSampleTypeService.findAll(pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }

    /**
     * {@code GET /admin/sample-types/:sampleTypeId} : get the "login" user.
     *
     ...
     */
    @GetMapping("/sample-types/{testId}")
    public ResponseEntity<SampleType> getSampleType(@PathVariable String sampleTypeId) {
        log.debug("REST request to get test : {}", sampleTypeId);
        return ResponseUtil.wrapOrNotFound(limsSampleTypeService.findById(sampleTypeId));
    }
